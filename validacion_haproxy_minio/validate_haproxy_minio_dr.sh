...
#  Entorno: DATACENTER ALTERNO (DR)
#  Nodos   : pbigd-stg01-cont | pbigd-stg02-cont | pbigd-stg03-cont  (co-ubicados con MinIO DR)
#
...
#  Keepalived delante de MinIO) ya esté implementada, ni en Principal ni en
#  DR. En context/Contexto-extra-correos.md (2026-08-01, punto 1) aparece
#  únicamente como una RECOMENDACIÓN de Bluepoint a la Cooperativa, sin
#  script de implementación asociado (a diferencia del caso PgBouncer/
#  Postgres, que sí tiene 01_configuracion_keepalived_vip_pgbouncer.sh).
#  Además, logs/flink/dr/*.txt registra a Flink DR alcanzando MinIO DR
#  directamente en "http://pbigd-stg01-cont:9000" — un solo nodo, sin VIP —
#  evidencia adicional de que, si esta capa existe, al menos Flink DR
#  todavía no la usa.
#
...
#  patrón que validacion_minio/validate_minio_dr.sh con
#  CONTAINER_PATTERN="minio"). Si algo no se encuentra, se reporta como
#  FAIL/WARN "no implementado / no encontrado", no se inventa.
#
#  Diferencia de diseño a documentar (no asumir omisión): a diferencia de
#  otros componentes (p. ej. Flink, que en DR corre con menos TaskManagers
#  que en Principal), el inventario de nodos MinIO DR
#  (context/hostnames.txt) lista los 3 nodos completos —stg01/02/03-cont—
#  sin reducción. Si en la práctica DR corre con menos redundancia en esta
#  capa (p. ej. 2 nodos HAProxy/Keepalived en vez de 3), este script lo
#  reportará como discrepancia real entre 1.1/1.4 y el inventario, no como
#  fallo asumido de antemano.
#
#  Uso:
#    chmod +x validate_haproxy_minio_dr.sh
#    ./validate_haproxy_minio_dr.sh [opciones]
#
#  Opciones:
...
#    --mc-alias <alias>   Alias de mc apuntando a la VIP (default: minio-vip-dr)
...
# ── Parámetros ─────────────────────────────────────────────────────────────────
ENV_LABEL="DR (Datacenter Alterno)"
NODES=("pbigd-stg01-cont" "pbigd-stg02-cont" "pbigd-stg03-cont")
S3_PORT=9000
CONSOLE_PORT=9001
